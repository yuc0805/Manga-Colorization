...
import cv2
# project
from Manga_Panel_Extractor.utils import get_files, load_image
...
class PanelExtractor:
    ...
    def extract(self, folder,output_dir = 'panels_output'):
        ...
        for i, img in tqdm(enumerate(paperless_imgs), desc="extracting panels"):
            ...
            font = ImageFont.truetype('Manga_Panel_Extractor/extractor/Open-Sans-Bold.ttf', 160)
    ...
    def concatPanels(self, img_file, fake_imgs, masks, panel_masks,
                     out_folder = '/Users/leo/Desktop/Manga-Colorization/cyclegan_extractor_result'):
        img = io.imread(img_file)
        print('concat input shape',img.shape)
        if len(img.shape) == 2:
        ...
        for i in range(len(fake_imgs)):
            ...
            print('panel_mask shape',fake_img.shape)
        ...
        print(f"Resized final image to 256x256: {img.shape}")
    ...
    #             print(f"Invalid coordinates for panel {i}, skipping this panel.")
    #             continue
    ...
    #         # Ensure the region in the original image matches the size of the mask and fake image
    #         img_section = img[y:y + h, x:x + w]
    #         if img_section.shape[:2] != resized_panel_mask.shape:
    #             print(f"Adjusting dimensions: img_section {img_section.shape[:2]} vs resized_panel_mask {resized_panel_mask.shape}")
    #             # Resize the mask to match the image section if necessary
    #             resized_panel_mask = resize(resized_panel_mask, img_section.shape[:2], preserve_range=True).astype(np.uint8)
    #             resized_fake_img = resize(resized_fake_img, img_section.shape[:2], preserve_range=True).astype(np.uint8)

    #         print(f"Final Resized panel mask shape: {resized_panel_mask.shape}")
    #         print(f"Final Resized fake image shape: {resized_fake_img.shape}")
    ...
    #     io.imsave(out_img_path, img)
    #     print(f"Saved final image to {out_img_path}")
    ...
    #     # Determine output paths
    #     out_folder = '/Users/leo/Desktop/Manga-Colorization/cyclegan_extractor_result' #os.path.dirname(img_file)
    #     out_name = os.path.basename(img_file)
    #     out_name = os.path.splitext(out_name)[0]
    #     out_img_path = os.path.join(out_folder, 'color', f'{out_name}_color.png')
